...
public class BookStoreRunner {
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("    create             - add a new book to the book store");
        ...
        while (true) {
            ...
            if (input.startsWith("list")) {
                String searchString = substring(input, "list".length());
                Book[] books = bookList.list(searchString);
                lastBookListResult = books;
                printBooks(books);
            }

            if (input.startsWith("basket")) {
                String command = scanner.next();

                if (command.equals("add")) {
                    int id = parseInt(scanner.next());
                    Book book = lastBookListResult[id]; // TODO NPE
                    basket.addBook(book);
                }
